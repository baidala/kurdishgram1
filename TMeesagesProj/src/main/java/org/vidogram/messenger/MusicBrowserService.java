  ...
  implements NotificationCenter.NotificationCenterDelegate
{
  ...
  private void loadChildrenImpl(String paramString, MediaBrowserService.Result<List<MediaBrowser.MediaItem>> paramResult)
  {
    ...
    if ("__ROOT__".equals(paramString))
    {
      ...
      localBuilder = new MediaDescription.Builder().setMediaId("__CHAT_" + j);
      if (j > 0)
      {
        paramString = (TLRPC.User)this.users.get(Integer.valueOf(j));
        if (paramString != null)
        {
          localBuilder.setTitle(ContactsController.formatName(paramString.first_name, paramString.last_name));
          if ((paramString.photo == null) || (!(paramString.photo.photo_small instanceof TLRPC.TL_fileLocation)))
            break label273;
          paramString = paramString.photo.photo_small;
          if (paramString == null)
            break label530;
    ...
    label273: label530: for (Object localObject1 = localObject2; ; localObject1 = null)
    {
      ...
      paramString = null;
      break label150;
      paramString = (TLRPC.Chat)this.chats.get(Integer.valueOf(-j));
      if (paramString != null)
      {
        localBuilder.setTitle(paramString.title);
        if ((paramString.photo != null) && ((paramString.photo.photo_small instanceof TLRPC.TL_fileLocation)))
        {
          paramString = paramString.photo.photo_small;
          break label150;
      ...
      if ((paramString != null) && (paramString.startsWith("__CHAT_")))
        try
        {
          i = Integer.parseInt(paramString.replace("__CHAT_", ""));
  ...
  public void onLoadChildren(String paramString, MediaBrowserService.Result<List<MediaBrowser.MediaItem>> paramResult)
  {
    if (!this.chatsLoaded)
    {
      ...
      MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable(paramString, paramResult)
      {
        public void run()
        {
          ...
          while (true)
          {
            AndroidUtilities.runOnUIThread(new Runnable()
            {
              public void run()
              {
                ...
                while (true)
                {
                  ...
                  return;
                  label361: MusicBrowserService.this.mediaSession.setQueueTitle("DELETED USER");
                  continue;
                  label379: localObject2 = (TLRPC.Chat)MusicBrowserService.this.chats.get(Integer.valueOf(-MusicBrowserService.this.lastSelectedDialog));
                  if (localObject2 != null)
                  {
            ...
            break;
            label134: localException.dispose();
