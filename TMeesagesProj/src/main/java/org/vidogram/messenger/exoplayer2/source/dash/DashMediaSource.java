...
import org.vidogram.messenger.exoplayer2.source.dash.manifest.DashManifestParser;
...
import org.vidogram.messenger.exoplayer2.source.dash.manifest.UtcTimingElement;
  ...
  implements MediaSource
{
  ...
  private DashMediaSource(DashManifest paramDashManifest, Uri paramUri, DataSource.Factory paramFactory, DashManifestParser paramDashManifestParser, DashChunkSource.Factory paramFactory1, int paramInt, long paramLong, Handler paramHandler, AdaptiveMediaSourceEventListener paramAdaptiveMediaSourceEventListener)
  {
    ...
    label141: for (boolean bool1 = bool2; ; bool1 = false)
    {
    ...
    label147: this.manifestCallback = new ManifestCallback(null);
  ...
  private void processManifest(boolean paramBoolean)
  {
    ...
    PeriodSeekInfo localPeriodSeekInfo = PeriodSeekInfo.createPeriodSeekInfo(this.manifest.getPeriod(i), this.manifest.getPeriodDurationUs(i));
    long l2 = ((PeriodSeekInfo)localObject).availableStartTimeUs;
    long l3 = localPeriodSeekInfo.availableEndTimeUs;
    long l1;
    if ((this.manifest.dynamic) && (!localPeriodSeekInfo.isIndexExplicit))
    {
      l3 = Math.min(getNowUnixTimeUs() - C.msToUs(this.manifest.availabilityStartTime) - C.msToUs(this.manifest.getPeriod(i).startMs), l3);
      l1 = l2;
      if (this.manifest.timeShiftBufferDepth != -9223372036854775807L)
      {
        l1 = l3 - C.msToUs(this.manifest.timeShiftBufferDepth);
        while ((l1 < 0L) && (i > 0))
        {
    ...
    while (true)
    {
      ...
      j = 0;
      while (true)
        if (j < this.manifest.getPeriodCount() - 1)
        {
          l3 += this.manifest.getPeriodDurationUs(j);
          j += 1;
          continue;
          l1 = this.manifest.getPeriodDurationUs(0);
          break;
        }
      l1 = 0L;
      long l4;
      if (this.manifest.dynamic)
      {
        ...
        if (l4 == -1L)
        {
          if (this.manifest.suggestedPresentationDelay == -9223372036854775807L)
            break label544;
          l1 = this.manifest.suggestedPresentationDelay;
        }
      }
      while (true)
      {
  ...
  private void scheduleManifestRefresh()
  {
    ...
    if (l2 == 0L)
      l1 = 5000L;
    l1 = Math.max(0L, l1 + this.manifestLoadStartTimestamp - SystemClock.elapsedRealtime());
    ...
    private long getAdjustedWindowDefaultStartPositionUs(long paramLong)
    {
      ...
      do
      {
        ...
        do
        {
          ...
          l2 = this.offsetInFirstPeriodUs;
          paramLong = this.manifest.getPeriodDurationUs(0);
          l2 += l1;
          i = 0;
          while ((i < this.manifest.getPeriodCount() - 1) && (l2 >= paramLong))
          {
            i += 1;
            l3 = this.manifest.getPeriodDurationUs(i);
            l2 -= paramLong;
            paramLong = l3;
          }
          localObject = this.manifest.getPeriod(i);
      ...
      return ((DashSegmentIndex)localObject).getTimeUs(((DashSegmentIndex)localObject).getSegmentNum(l2, paramLong)) + l1 - l2;
    }
    ...
    implements ParsingLoadable.Parser<Long>
  {
    public Long parse(Uri paramUri, InputStream paramInputStream)
    {
      ...
      catch (java.text.ParseException paramUri)
      {
      }
  ...
  private static final class PeriodSeekInfo
  {
    ...
    public static PeriodSeekInfo createPeriodSeekInfo(Period paramPeriod, long paramLong)
    {
      ...
      while (i < j)
      {
        ...
        if (m != -1)
        {
          l2 = localDashSegmentIndex.getTimeUs(m);
          l2 = Math.min(l1, localDashSegmentIndex.getDurationUs(m, paramLong) + l2);
        }
        i += 1;
        l1 = l2;
      }
 ...
 * Qualified Name:     org.vidogram.messenger.exoplayer2.source.dash.DashMediaSource
