...
import org.vidogram.messenger.exoplayer2.util.NalUnitUtil.PpsData;
import org.vidogram.messenger.exoplayer2.util.NalUnitUtil.SpsData;
import org.vidogram.messenger.exoplayer2.util.ParsableByteArray;
import org.vidogram.messenger.exoplayer2.util.ParsableNalUnitBitArray;
  ...
  implements ElementaryStreamReader
{
  ...
  private void endNalUnit(long paramLong1, int paramInt1, int paramInt2, long paramLong2)
  {
    ...
    while (true)
    {
      ...
      return;
      label317: if (this.sps.isCompleted())
      {
  ...
  private static final class SampleReader
  {
    ...
    public void appendToNalUnit(byte[] paramArrayOfByte, int paramInt1, int paramInt2)
    {
      ...
      while (true)
      {
        ...
        if (!this.bitArray.canReadBits(localSpsData.frameNumLength))
          continue;
        bool2 = false;
        boolean bool5 = false;
        boolean bool6 = false;
        ...
        if (localSpsData.picOrderCountType != 0)
          break label595;
        if (!this.bitArray.canReadBits(localSpsData.picOrderCntLsbLength))
          continue;
        ...
        if (!this.bitArray.canReadExpGolombCodedNum())
          continue;
        j = this.bitArray.readSignedExpGolombCodedInt();
        k = n;
        paramInt2 = i2;
        i = i1;
      }
 ...
 * Qualified Name:     org.vidogram.messenger.exoplayer2.extractor.ts.H264Reader
