...
public class StreamingDrmSessionManager<T extends ExoMediaCrypto>
  implements DrmSession<T>, DrmSessionManager<T>
{
  ...
  private void onError(Exception paramException)
  {
    ...
    if ((this.eventHandler != null) && (this.eventListener != null))
      this.eventHandler.post(new Runnable(paramException)
      {
        public void run()
        {
  ...
  private void onKeyResponse(Object paramObject)
  {
    ...
    while (true)
    {
      ...
      try
      {
        this.mediaDrm.provideKeyResponse(this.sessionId, (byte[])(byte[])paramObject);
        this.state = 4;
        if ((this.eventHandler == null) || (this.eventListener == null))
          continue;
  ...
  private void onProvisionResponse(Object paramObject)
  {
    ...
    try
    {
      this.mediaDrm.provideProvisionResponse((byte[])(byte[])paramObject);
      if (this.state == 2)
      {
        openInternal(false);
        return;
    ...
    catch (android.media.DeniedByServerException paramObject)
    {
      onError(paramObject);
      return;
    }
  ...
  public DrmSession<T> acquireSession(Looper paramLooper, DrmInitData paramDrmInitData)
  {
    ...
    while (true)
    {
      ...
      openInternal(true);
      return this;
      label219: this.schemeData = new DrmInitData.SchemeData(C.WIDEVINE_UUID, this.schemeData.mimeType, paramLooper);
    }
  }
 ...
 * Qualified Name:     org.vidogram.messenger.exoplayer2.drm.StreamingDrmSessionManager
