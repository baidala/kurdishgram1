...
public class Input
{
  ...
  private void paintPath(Path paramPath)
  {
    ...
    this.renderView.getPainting().paintStroke(paramPath, this.clearBuffer, new Runnable(paramPath)
    {
      public void run()
      {
  ...
  private Point smoothPoint(Point paramPoint1, Point paramPoint2, Point paramPoint3, float paramFloat)
  {
    ...
    return new Point(paramPoint1.x * d1 + paramPoint3.x * d2 + paramPoint2.x * d3, d1 * paramPoint1.y + d2 * paramPoint3.y + paramPoint2.y * d3, 1.0D);
  }
  ...
  private void smoothenAndPaintPoints(boolean paramBoolean)
  {
    if (this.pointsCount > 2)
    {
      ...
      paintPath(new Path(localObject2));
    ...
    paintPath(new Path(localObject1));
  }
 ...
 * Qualified Name:     org.vidogram.ui.Components.Paint.Input
