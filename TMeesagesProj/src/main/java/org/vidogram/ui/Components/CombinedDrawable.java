  ...
  implements Drawable.Callback
{
  ...
  public void draw(Canvas paramCanvas)
  {
    ...
    while (true)
    {
      ...
      if (this.iconWidth != 0)
      {
        i = getBounds().centerX() - this.iconWidth / 2 + this.left;
        j = getBounds().centerY() - this.iconHeight / 2 + this.top;
        this.icon.setBounds(i, j, this.iconWidth + i, this.iconHeight + j);
        continue;
      }
      int i = getBounds().centerX() - this.icon.getIntrinsicWidth() / 2 + this.left;
      int j = getBounds().centerY() - this.icon.getIntrinsicHeight() / 2 + this.top;
 ...
 * Qualified Name:     org.vidogram.ui.Components.CombinedDrawable
