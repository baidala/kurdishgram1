...
import org.vidogram.ui.Components.Crop.CropRotationWheel;
import org.vidogram.ui.Components.Crop.CropRotationWheel.RotationWheelListener;
import org.vidogram.ui.Components.Crop.CropView;
import org.vidogram.ui.Components.Crop.CropView.CropViewListener;
...
public class PhotoCropView extends FrameLayout
{
  ...
  public float getLimitY()
  {
    ...
    {
      float f = i;
      return this.rectY - Math.max(0.0F, (float)Math.ceil((f + (getHeight() - this.bitmapHeight * this.bitmapGlobalScale)) / 2.0F));
    }
  }
  ...
  public void moveToFill(boolean paramBoolean)
  {
    ...
    label259: label290: for (int i = AndroidUtilities.statusBarHeight; ; i = 0)
    {
      float f7 = i;
      f2 = (getWidth() - f4) / 2.0F;
      ...
      float f9 = this.rectY;
      this.delegate.needMoveImageTo(f4 * (f1 - 1.0F) + f2 + (f5 - f6) * f1, f7 * (f1 - 1.0F) + f3 + (f8 - f9) * f1, f1 * this.bitmapGlobalScale, paramBoolean);
      return;
      break;
      f1 = f2;
      if (f2 >= 1.0F)
  ...
  public void setAnimationProgress(float paramFloat)
  {
    ...
    while (true)
    {
      invalidate();
      return;
      label72: this.rectX = (this.animationStartValues.left + (this.animationEndValues.left - this.animationStartValues.left) * paramFloat);
  ...
  public void setBitmap(Bitmap paramBitmap, int paramInt, boolean paramBoolean)
  {
    ...
    if (this.cropView == null)
    {
      ...
      {
        public void onAspectLock(boolean paramBoolean)
        {
          PhotoCropView.this.wheelView.setAspectLock(paramBoolean);
        }

        public void onChange(boolean paramBoolean)
        {
          if (PhotoCropView.this.delegate != null)
            PhotoCropView.this.delegate.onChange(paramBoolean);
        }
      });
      ...
      this.wheelView.setListener(new CropRotationWheel.RotationWheelListener()
      {
        ...
        {
          PhotoCropView.this.cropView.setRotation(paramFloat);
          if (PhotoCropView.this.delegate != null)
            PhotoCropView.this.delegate.onChange(false);
        }

        public void onEnd(float paramFloat)
        {
 ...
 * Qualified Name:     org.vidogram.ui.Components.PhotoCropView
