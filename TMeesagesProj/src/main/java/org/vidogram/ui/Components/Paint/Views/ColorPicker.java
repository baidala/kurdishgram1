...
public class ColorPicker extends FrameLayout
{
  ...
  public ColorPicker(Context paramContext)
  {
    ...
    this.shadowDrawable = getResources().getDrawable(2130837882);
    ...
    this.settingsButton.setImageResource(2130838003);
  ...
  private int interpolateColors(int paramInt1, int paramInt2, float paramFloat)
  {
    ...
    float f = k;
    k = Math.min(255, (int)((m - k) * paramFloat + f));
    i = Math.min(255, (int)(i + (j - i) * paramFloat));
    f = paramInt1;
    return Color.argb(255, k, i, Math.min(255, (int)(paramFloat * (paramInt2 - paramInt1) + f)));
  }
  ...
  public int colorForLocation(float paramFloat)
  {
    ...
    while (true)
    {
      float f1 = LOCATIONS[j];
      j = COLORS[j];
      float f2 = LOCATIONS[i];
      return interpolateColors(j, COLORS[i], (paramFloat - f1) / (f2 - f1));
      i += 1;
      break;
      i = -1;
    }
  ...
  protected void onDraw(Canvas paramCanvas)
  {
    ...
    if (this.changingWeight)
      f1 = this.weight * AndroidUtilities.dp(190.0F);
    while (true)
    {
      ...
      this.shadowDrawable.setBounds(i - k, j - k, i + k, k + j);
  ...
  protected void onLayout(boolean paramBoolean, int paramInt1, int paramInt2, int paramInt3, int paramInt4)
  {
    ...
    this.rectF.set(paramInt4, i, paramInt4 + AndroidUtilities.dp(8.0F), i + paramInt3);
 ...
 * Qualified Name:     org.vidogram.ui.Components.Paint.Views.ColorPicker
